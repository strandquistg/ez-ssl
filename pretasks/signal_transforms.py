...
#Slightly adapted for ECoG data strcture, taken from "Self-supervised ECG Representation Learning for Emotion Recognition" repo on GitLab 

import numpy as np
import math, pdb, glob, os, sys, natsort, h5py
import cv2
...
def add_noise(signal, noise_amount):
    ...
    adding noise
    """
    noise = np.random.normal(1, noise_amount, (np.shape(signal)[0], np.shape(signal)[1], np.shape(signal)[-1]))
